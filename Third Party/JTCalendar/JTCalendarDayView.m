...
- (void)reloadData
{
    __weak JTCalendarDayView *weakSelf = self;
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        dotView.hidden = ![weakSelf.calendarManager.dataSource calendarHaveEvent:weakSelf.calendarManager date:weakSelf.date];
        dispatch_sync(dispatch_get_main_queue(), ^{
            BOOL selected = [weakSelf isSameDate:[weakSelf.calendarManager currentDateSelected]];
            [weakSelf setSelected:selected animated:NO];
        });
    });
}
